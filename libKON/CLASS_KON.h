...
//		int KARAR	  (const double* _In, const int &_oEMIR,const MY::uint &_i,const MY::uint &_id,MY::uint &_info);
	int KARAR(const double *_In, const int &_oEMIR, const MY::uint &_i,
			MY::uint &_info);
	void KARAR_MLP(const double *_In, int &_oEMIR, const MY::uint &_i,
			double &_net);
	void KARAR_SLP(const double *_In, int &_oEMIR, const MY::uint &_i,
			double &_net);
	void KARAR_RBF(const double *_In, int &_oEMIR, const MY::uint &_i,
			double &_net);
	void KARAR_LVQ(const double *_In, const int &_oEMIR, const MY::uint &_i,
...
public:
	...
	static void OKU_DATA(std::string name,
