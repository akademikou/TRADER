...
/***********************************************************************************/
KON::~KON() {
	if(OHLC !=NULL){Free_Matrix_2D(OHLC); OHLC=NULL;}
	if(DATA !=NULL){Free_Matrix_2D(DATA); DATA=NULL;}
	if(EMIR !=NULL){Free_Matrix_2D(EMIR); EMIR=NULL;}
	if(tRSI !=NULL){Free_Matrix_1D(tRSI); tRSI=NULL;}
}
/***********************************************************************************/
KON::KON() {
	W_RBF=NULL;
...
//	oADX	oARN	oOSC		        													[40 - 42]
	...
	for(MY::uint i=0;i<nDATA;i++){
		for(MY::uint j=0;j<6   ;j++){OHLC[i][j]=rawDATA[i][j+1];}
		for(MY::uint j=0;j<nINP;j++){DATA[i][j]=TI_DATA[i][j];}
...
//	}
	...
	for(MY::uint i=0;i<nDATA;i++){OHLC[i][3]/=NORM;}
	...
	for(MY::uint i=0;i<nTH;i++){
		...
		EXT[i]=0;
		YSK[i]=0;
		for(MY::uint j=0;j<nDATA;j++){EMIR[i][j]=0;}
	}
	nDIM = nH*nINP;
/*	nKAT = 2;
...
/**************************************************/
	...
	for (MY::uint j = 0; j < (MY::uint)_DATA[0].size(); j++) {
		...
		for(MY::uint i=0;i<(MY::uint)_DATA.size();i++){
			if(ENB < _DATA[i][j]){ENB = _DATA[i][j];}
			if(ENK > _DATA[i][j]){ENK = _DATA[i][j];}
		}
		ENK=fabs(ENK);
		if(ENB<ENK) {ENB=ENK;}
		for(MY::uint i=0;i<(MY::uint)_DATA.size();i++){
			if(ENB!=0){_DATA[i][j]=(100*_DATA[i][j]/ENB);}
		}
	}
}
/***********************************************************************************/
void KON::INIT_RBF(MY::uint id){
	...
	if(W_RBF !=NULL){Free_Matrix_2D(W_RBF); W_RBF=NULL;}
	if(C_RBF !=NULL){Free_Matrix_2D(C_RBF); C_RBF=NULL;}
	W_RBF	= Init_Matrix_2D<double>(nDATA,c);
	C_RBF	= Init_Matrix_2D<double>(c,dim);

	double **_X=Init_Matrix_2D<double>(n,dim);
	for(i=0;i<n;i++){for (j=0;j<dim;j++){_X[i][j]=DATA[nILK+i][j];}}
	FLC myFLC(n,c,dim,_X);
	myFLC.RUN();
	myFLC.YAZ(id);
	for (i=0;i<n;i++){for (j=0;j<c;j++){W_RBF[nILK+i][j]=myFLC.W[i][j];}}
	for (i=0;i<c;i++){for (j=0;j<dim;j++){C_RBF[i][j]=myFLC.C[i][j];}}
	if(_X !=NULL){Free_Matrix_2D(_X); _X=NULL;}
}
/***********************************************************************************/
void KON::KARAR_MLP(const double* _In, int &_oEMIR,const MY::uint &_i,double &_net){
	double L=0.2;	// 2*L=0.2
	double IO[8][30];
	for (MY::uint j=0; j<nGIR[0]-2; j++) { IO[0][j] = DATA[_i][j];}
	for (MY::uint i=0; i<nKAT; i++) {
		IO[i][nGIR[i]-2] = 100.0;
		IO[i][nGIR[i]-1] = double(_oEMIR*100.0);
		for (MY::uint j=0; j<nCIK[i]; j++) {
			double net = 0;
			for (MY::uint k=0; k<nGIR[i]; k++) {net+=(_In[MAP[i][j][k]]*IO[i][k]);}
			IO[i+1][j] = 100*(2/(1+exp(-L*net))-1);
		}
	}
//	_net= 100*(IO[nKAT][1]-IO[nKAT][0]);
	     if(IO[nKAT][0]>IO[nKAT][1] && IO[nKAT][0]>IO[nKAT][2]){_net=  100;}
	else if(IO[nKAT][1]>IO[nKAT][0] && IO[nKAT][1]>IO[nKAT][2]){_net=    0;}
	else if(IO[nKAT][2]>IO[nKAT][0] && IO[nKAT][2]>IO[nKAT][1]){_net= -100;}else{_net=0;}
}
/***********************************************************************************/
void KON::KARAR_SLP(const double* _In, int &_oEMIR,const MY::uint &_i,double &_net) {
	_net=100.0*(_In[nDIM-2]+double(_oEMIR)*_In[nDIM-1]);
	for(MY::uint j=0;j<nINP;j++){_net+=DATA[_i][j]*_In[j];}
}
/***********************************************************************************/
void KON::KARAR_RBF(const double* _In, int &_oEMIR,const MY::uint &_i,double &_net) {
	_net=100.0*(_In[nDIM-2]+double(_oEMIR)*_In[nDIM-1]);
	for(MY::uint j=0;j<nDIM-2;j++){_net+=100*W_RBF[_i][j]*_In[j];}
}
/***********************************************************************************/
void KON::KARAR_LVQ(const double* _In, const int &_oEMIR,const MY::uint &_i,double &_net,MY::uint &_info) {
	double ENK  = 1e100;
	MY::uint ind = 0;
	MY::uint ILK = 0;
	MY::uint SON = nINP;		
	if constexpr (VER == 1){SON = nINP;}
	if constexpr (VER == 2){SON = nINP-1;}
	if constexpr (VER == 3){
			 if(_oEMIR == -1 ){ ILK = nINP*0/3; SON = nINP*1/3;}
		else if(_oEMIR ==  0 ){ ILK = nINP*1/3; SON = nINP*2/3;}
		else if(_oEMIR ==  1 ){ ILK = nINP*2/3; SON = nINP*3/3;}
	}	
	for (MY::uint i = 0; i < nH; i++) {
		...
		if constexpr (VER == 2){L1 += ((_oEMIR*100- (*VERI_2)) * (_oEMIR*100- (*VERI_2)));}
		if (L1<ENK) { ind = i; ENK=L1; }
	}
	_info = ind;
	_net = MY::uint (ind / nPSS);
	     if(_net == 0){_net = (-1);}
	else if(_net == 2){_net = ( 1);}
	else              {_net = ( 0);}
}
/***********************************************************************************/
void KON::UYGUNLUK_FAST_LVQ	( const double* _In, double &_result){
	...
	for(MY::uint P=(nILK+1);P<(nSON-1);P++){
		...
		if constexpr (VER == 2){SON = nINP-1;}
		if constexpr (VER == 3){
				 if(oEMIR == -1 ){ ILK = nINP*0/3; SON = nINP*1/3;}
			else if(oEMIR ==  0 ){ ILK = nINP*1/3; SON = nINP*2/3;}
			else if(oEMIR ==  1 ){ ILK = nINP*2/3; SON = nINP*3/3;}
		}	
		for (MY::uint i = 0; i < nH; i++) {
			double L1 = 0;
			...
			if constexpr (VER == 2){L1 += ((oEMIR*100- (*VERI_2)) * (oEMIR*100- (*VERI_2)));}
			if (L1<ENK) { ind = i; ENK=L1; }
		}
		/*********************************************/
		...
		if(P>=1 && nEMIR != oEMIR){
			...
			if(nEMIR > oEMIR){			// Bu koşullarda ALMA !!!
				if( new_RSI > 60 || new_RSI < 20 || new_RSI < old_RSI){nEMIR = oEMIR;}
			}else{							// Bu koşullarda SATMA !!!
				if( new_RSI < 40 || new_RSI > 80 || new_RSI > old_RSI){nEMIR = oEMIR;}
			}
		}else{nEMIR = oEMIR;}
		#endif
		/*********************************************/
		if(oEMIR!=nEMIR){
			...
			if(D==0.0){std::cout<<"HATA\n";system("pause");}
//			if(ILK_PARA == 0){ILK_PARA=SON_PARA=D;}
			if(nEMIR == 0){ADET = (-SON_ADET);}// paranin tamami kadar al/sat
			else{
				if(nEMIR == 1){ADET = (nEMIR*SON_PARA/D)/(1+TxnFees);}
				else{ADET = (-2*SON_ADET+(nEMIR*SON_PARA/D))/(1+TxnFees);}
			}
		...
		if(P == (nSON-100)){ARA_PARA = SON_PARA+SON_ADET*OHLC[P][3];}
		oEMIR=nEMIR;
		/*********************************************/
	}
	if(SON_ADET != 0){
...
//		_result[0]= -RET;
		...
		if(K1>0.00 && K2>0.00 && K2>(K1*0.1)){(&_result)[0]= -K1;}
		else{(&_result)[0]= (-K1+1);}
			 (&_result)[1]= (-K2);
	}
}
...
int  KON::KARAR(const double* _W, const int &_oEMIR,const MY::uint &_i,MY::uint &_info) {
	if(_i==nSON-1){return 0;}
	double net=0;
	int ORD1=0;
...
//	KARAR_RBF(_W, _oEMIR,_i,net);
	...
	if(net==0){ORD1=_oEMIR;}else{ORD1  = EMIR_YAZ(net);}
...
int  KON::EMIR_YAZ(const double &_net){
       if(_net > ( 0.001)){return ( 1);}
  else if(_net < (-0.001)){return (-1);}else{return  0;}
}
/***********************************************************************************/
...
void KON::UYGUNLUK( const double* _In, double &_result,bool _VERBOSE) {
	if(!_VERBOSE){UYGUNLUK_FAST_LVQ	(_In,_result); return;}
	std::cout<<"UYGUNLUK\n";
	...
	_result=0;
	double S=0;
	for(MY::uint i=0;i<nDATA;i++){EMIR[id][i]=0;}
//	int YON=0;
//	for(int K=-1;K<=1;K++)
	int K=0;
	{
...
//		if(i==(nSON-1)){YON=0;}
		if(LAST!=YON){
			...
			nTXN++;
				 if(FARK>0){gross_profits += FARK;		nWIN++;}
			else if(FARK<0){gross_loss    += fabs(FARK);nLOS++;}
			else{nNTR++;}
			if(Big_WIN<FARK){Big_WIN=FARK;}
			if(Big_LOS>FARK){Big_LOS=FARK;}
			SD_1+=(FARK*FARK);
			SD_2+=(FARK);
//			if(VERBOSE){std::cout<<i<<"\t"<<FARK<<"\t"<<WALLET[1]<<"\t"<<WALLET[0]<<"\n";}
	...
	*/
		LAST=YON;
		if(i == (nSON-100)){ARA_PARA = SON_PARA+SON_ADET*OHLC[i][3];}
	}
	if(SON_ADET!=0){
		std::cout<<"-------------"<<SON_ADET<<std::endl;
		if(SON_ADET>0){SON_PARA  = SON_PARA+SON_ADET*OHLC[nSON-1][3]*(1-TxnFees);}
		if(SON_ADET<0){SON_PARA  = SON_PARA-SON_ADET*OHLC[nSON-1][3]*(1-TxnFees);}
		nTXN++;
	}
...
//	}
	...
	SD = (nTXN==0)		? 0 : sqrt((SD_1-2*Average_Profit*SD_2+Average_Profit*Average_Profit*nTXN)/nTXN);
	double Sharp_Ratio = (SD==0) ? 0: Average_Profit / SD;

//	if(nTXN<5 || nWIN < nLOS || Average_Profit < 0){_result=1E20;}else{
	if(nTXN < 5){_result=1E5;}else{
		_result= Net_Profit;
...
*/
/***********************************************************************************/
void KON::OKU_DATA(std::string name,std::vector<std::vector<double>>&rawDATA) {
	double dTmp;
	std::string line;
...
//	double _result=0;
	...
	MY::uint info;
	MY::uint SAY[nTH][nH];
	for(MY::uint i=0;i<nTH;i++){for(MY::uint j=0;j<nH;j++){SAY[i][j]=0;}}
	for(MY::uint i=(nILK+1);i<nSON;i++){
		EMIR[id][i]=KARAR(_In,EMIR[id][i-1],i,info);
//		EMIR[id][i]=KARAR(_In,EMIR[id][i-1],i,id,info);
		SAY[id][info]++;
...
//	for (MY::uint j = 0; j < nH; j++) { std::cout << SAY[id][j]<<"\n"; }
	...
	for(MY::uint i=0;i<nDATA;i++){DOSYA<<EMIR[id][i]<<"\t"<<OHLC[i][3]<<"\n";}
	DOSYA.close();
}
/***********************************************************************************/
...
//	double _result=0;
	...
	for(MY::uint i=0;i<(MY::uint)PSO_DATA.size();i++){
		for(MY::uint j=0;j<(MY::uint)PSO_DATA[i].size();j++){PSO_RES[i][j]=PSO_DATA[i][j];}
		PSO_DATA[i].clear();
	}
	PSO_DATA.clear();
	double* _In;
...
//			EMIR[id][j]=KARAR(_In,EMIR[id][j-1],j,id,info);
	...
	EMIR[id][nSON-1]=0;
	if(PSO_RES !=NULL){Free_Matrix_2D(PSO_RES); PSO_RES=NULL;}
	double _result=0;
	UYGUNLUK_1(EMIR[id],_result,true);
	DOSYA.open(NAME, std::ios::out);
	for(MY::uint i=0;i<nDATA;i++){DOSYA<<EMIR[id][i]<<"\t"<<OHLC[i][3]<<"\n";}
...
void KON::YAZ_OPT(std::string NAME, double* _In) {
	...
	for(MY::uint i=0;i<nDIM;i++){DOSYA<<_In[i]<<"\t";}
	DOSYA<<_In[nDIM];
	DOSYA<<"\t"<<nILK;
	DOSYA<<"\t"<<nSON;
	DOSYA<<std::endl;
...
/***********************************************************************************/
MY::uint KON::GET_DIM() {return (nDIM);}
/***********************************************************************************/
void KON::SET_LIM(const MY::uint _ILK,const MY::uint _SON) {nILK=_ILK;nSON=_SON;}
/***********************************************************************************/
void KON::GET_SNR(double* aSNR, double* uSNR) {
	for (MY::uint j = 0; j < nINP; j++) {
		...
		double ENK=  1E5;
		for(MY::uint i=(nILK+1);i<nSON;i++){			
			if(ENB < DATA[i][j]){ENB = DATA[i][j];}
			if(ENK > DATA[i][j]){ENK = DATA[i][j];}
		}
		for (MY::uint i = 0; i < nH; i++) {
