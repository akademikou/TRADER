...
LVQ::LVQ(int _nGIR, int _nCIK, int _nPSS, const double _OK) {
	dist_R_LVQ.param(std::uniform_real_distribution<double>::param_type { -100,
			100 });
	mt_A_LVQ.seed(
	...
	nH = nCIK * nPSS;
	D = NULL;
	D = Init_Matrix_1D<double>(nH);
	W = NULL;
	W = Init_Matrix_2D<double>(nH, nGIR);
	for (int i = 0; i < nH; i++) {
		D[i] = 0;
		for (int j = 0; j < nGIR; j++) {
			W[i][j] = dist_R_LVQ(mt_A_LVQ);
		}
	}
}
/*******************************************************************************/
...
void LVQ::TRAIN_TEST(const std::vector<std::vector<double>> _TEST_DATA,
		...
		const bool &TRAIN) {
	dist_I_LVQ.param(
			std::uniform_int_distribution<int>::param_type { _ILK, _SON - 1 });
	mt_A_LVQ.seed(
			static_cast<long unsigned int>(std::chrono::high_resolution_clock::now().time_since_epoch().count()));
	...
	for (int i = 0; i < 3; i++) {
		std::cout << i << "0: " << R[i][0] << " ("
				<< double(R[i][0]) / double(R[i][0] + R[i][1] + R[i][2])
				<< ")\t" << i << "1: " << R[i][1] << " ("
				<< double(R[i][1]) / double(R[i][0] + R[i][1] + R[i][2])
				<< ")\t" << i << "2: " << R[i][2] << " ("
				<< double(R[i][2]) / double(R[i][0] + R[i][1] + R[i][2])
				<< ")\n";
	}
	std::cout << "______________________________________" << std::endl;
	delete[] G_YSA;
