...
//		void* mem1 = new Tipus[size_m1];
	if (mem1 == NULL) {
		std::cout << size_m1 << " Init_Matrix_1D mem error" << std::endl;
		return NULL;
	}
	return (Tipus*) mem1;
}
...
;
/**************************************************************/
template<class Tipus> Tipus** To_2D(const int &nx, const unsigned int &ny,
		Tipus *p) {
	Tipus **X;
	unsigned int i, s = nx * ny;
...
/**************************************************************/
/**************************************************************/
template<class Tipus> Tipus** Init_Matrix_2D(const unsigned int &nx,
...
//		std::cout<<nx<<"\t"<<ny<<"\t"<<nx*ny<<"\t"<<N1*N2<<"\t"<<s<<"\n";
	X = (Tipus**) calloc(nx, sizeof(Tipus*));
	if (X == NULL) {
		std::cout << nx << " Init_Matrix_2D_1 mem error" << nx << "\t" << ny
				<< std::endl;
		return NULL;
	}
...
//		std::cout<<nx<<"\t"<<ny<<"\t"<<s<<"\n";
	if (p == NULL) {
		free(X);
		std::cout << ny << "  Init_Matrix_2D_2 mem error " << nx << "\t" << ny
				<< std::endl;
		return NULL;
	}
	for (i = 0; i < nx; i++) {
...
template<class Tipus> Tipus*** To_3D(const unsigned int &nx,
		const unsigned int &ny, const unsigned int &nz, Tipus *p) {
	Tipus ***X;
	unsigned int i, j, ii;
...
template<class Tipus> Tipus*** Init_Matrix_3D(const unsigned int &nx,
