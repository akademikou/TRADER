...
constexpr  const MY::uint _RAO	= 3;
constexpr  const MY::uint _GA	= 1;
constexpr  const MY::uint _GRD	= 1;
...
PSO::PSO(KON * _infunc, const MY::uint& _nI) {
	...
	for(MY::uint id=0;id<nProcessors;id++){mt_A_PSO[id]=Xoshiro256PlusPlus(AA+id*rand()); mt_A_PSO[id].jump() ;}
}
/***********************************************************************************/
PSO::~PSO() {
	delete []mt_A_PSO;
	...
	delete []uSNR;
	delete []araSNR;
	if (tmpP	!= NULL) { tmpP		= Free_Matrix_2D(tmpP);}
	...
	if (G_WRST	!= NULL) { G_WRST	= Free_Matrix_2D(G_WRST);}
...
void PSO::init_HAFIZA() {
	...
	for (MY::uint id = 0; id < nProcessors; id++) {
		for (MY::uint j =    0; j <  nDIM; j++) {G_BEST[id][j] = L_BEST[id][j] = 0;}
		for (MY::uint j = nDIM; j <= nTAM; j++) {G_BEST[id][j] = L_BEST[id][j] = MAX_VAL_PSO;}

		for (MY::uint j =    0; j <  nDIM; j++) {G_WRST[id][j] = 0;}
		for (MY::uint j = nDIM; j <= nTAM; j++) {G_WRST[id][j] = -MAX_VAL_PSO;}
	}
	#pragma omp parallel for
	for (int i = 0; i < (int)nPUB; i++){
		for (MY::uint j =    0; j <  nDIM; j++) {dPUB[i][j] = P_BEST[i][j] = VEL[i][j] =0;}
		for (MY::uint j = nDIM; j <= nTAM; j++) {dPUB[i][j] = P_BEST[i][j] = MAX_VAL_PSO;}
	}
		...
		do {
			...
			for (aDIM = 0; aDIM < nDIM; aDIM+=LDIM) {
				...
				for (MY::uint iter = 0; iter <= iterMAX; iter++) {					
					URET();
					bool TMP=COMPARE_UPDATE_B(G_BEST[nProcessors],G_BEST[0],true);
					if (TMP || iter % 20 == 0 )
					{
						if(TMP){GHANGED=true;}
						std::cout << iS << "\t"
...
//	std::cout<<"init_POP started\n";
	...
	if(use_gaussian==false){
		...
		for(MY::uint k = 0; k < 2; k++) {
			...
			if(!DOSYA.is_open()){std::cout<<PATH+NAME[k]<<" not opened\n";}else{
...
//						#endif
					...
					for(MY::uint j=0;j<iTMP;j++){dPUB[i][j]=vTmp[j];}
//					MY::uint s=i;
//					if(TEKRAR(dPUB,dPUB[i],0, s,nPUB,nDIM)==true){i--;}
	...
	#pragma omp parallel
	{
		...
		for (MY::uint i = nTH_BSL[id]+3; i < nTH_BTS[id]; i++) {
			if(i <  EK  ){continue;}
			if(i >= nPUB){break;}
			for (MY::uint j =   0; j <  aDIM; j++) { dPUB[i][j] = G_BEST[id][j]; }
			for (MY::uint j = SON; j <= nTAM; j++) { dPUB[i][j] = G_BEST[id][j]; }
//			for(MY::uint j=aDIM;j<SON;j++){
...
//			}
	...
	#pragma omp parallel
	{
		...
		for (MY::uint i = nTH_BSL[id]; i < nTH_BTS[id]; i++) {
			if(i >= nPUB){break;}
			for (MY::uint j =   0; j <  nDIM; j++) {VEL   [i][j] = 2*(DoubleFromBits(mt_A_PSO[id]())-0.5);}
			for (MY::uint j =   0; j <  aDIM; j++) {dPUB  [i][j] = G_BEST[0][j];}
			for (MY::uint j = SON; j <= nTAM; j++) {dPUB  [i][j] = G_BEST[0][j];}
			func->UYGUNLUK(dPUB[i],dPUB[i][nDIM],false);
			for (MY::uint j =   0; j <= nTAM; j++) {P_BEST[i][j] = dPUB[i][j];}
		}
	}
...
void PSO::inject_POP(){
	...
	for (MY::uint id= 1; id < nProcessors; id++) {
		for (MY::uint j = aDIM; j < SON; j++) {P_MEAN[0][j]+=P_MEAN[id][j];}
	}
	...
	#pragma omp parallel
	{
		...
		for (MY::uint i = nTH_BSL[id]; i < i_son; i+=2) {
			MY::uint k=i+1;
			if(i < 0 || k >= (nPUB-1)){break;}
			
			for (MY::uint j = aDIM; j < SON; j++) {
				dPUB[i][j]=G_BEST[0][j]+100*(DoubleFromBits(mt_A_PSO[id]())-0.5);
					 if(dPUB[i][j] < aSNR[j]){dPUB[i][j] = aSNR[j];}
				else if(dPUB[i][j] > uSNR[j]){dPUB[i][j] = uSNR[j];}

				dPUB[k][j]=P_MEAN[0][j]+100*(DoubleFromBits(mt_A_PSO[id]())-0.5);
					 if(dPUB[k][j] < aSNR[j]){dPUB[k][j] = aSNR[j];}
				else if(dPUB[k][j] > uSNR[j]){dPUB[k][j] = uSNR[j];}
			}
			func->UYGUNLUK(dPUB[i],dPUB[i][nDIM],false);
...
bool PSO::CRITERIA(const bool verbose) {
	...
	MY::uint SUM=0;
	for (MY::uint id = 0; id < nProcessors; id++) {
		for (MY::uint i = nTH_BSL[id]; i < nTH_BTS[id]; i++) {SUM++;}
	}
	if(SUM!=nPUB){std::cout<<"SUM NPUB HATASI "<<SUM<< "\t"<<nPUB<<"\n";}
...
//		system("pause");
	...
	if (sd<=1E-6 || (old_mean == mean && old_sd   == sd   && sd < fabs(mean)*0.001)){return true;}
	old_mean = mean;
	old_sd = sd;
	sum   [0] = 0;
	sq_sum[0] = 0;
...
/***********************************************************************************/
inline bool PSO::COMPARE_UPDATE_B(double* _In1,const double* _In2,bool UPDATE){
	if( _In1[nDIM  ] >  _In2[nDIM  ] ||
	   (_In1[nDIM  ] >= _In2[nDIM  ] &&
		_In1[nDIM+1] >  _In2[nDIM+1])){
		if(UPDATE){for (MY::uint j = 0; j <= nTAM; j++){_In1[j] =  _In2[j]; }}
		return true;
	}
	return false;
}
/***********************************************************************************/
inline bool PSO::COMPARE_UPDATE_W(double* _In1,const double* _In2,bool UPDATE){
	if( _In1[nDIM  ] <  _In2[nDIM  ] ||
	   (_In1[nDIM  ] <= _In2[nDIM  ] &&
		_In1[nDIM+1] <  _In2[nDIM+1])){
		if(UPDATE){for (MY::uint j = 0; j <= nTAM; j++){_In1[j] =  _In2[j]; }}
...
/***********************************************************************************/
inline bool PSO::NOTIFY(double* _In,const MY::uint &_id,const MY::uint &_i,const bool verbose){
...
#pragma omp critical
		{
		ALLERT(YESIL);
		std::cout <<  G_BEST[0][nDIM  ] << "\t"<<  G_BEST[_id][nDIM  ]  << "\t"<<  _In[nDIM  ] << "\n"
				  <<  G_BEST[0][nDIM+1] << "\t"<<  G_BEST[_id][nDIM+1]  << "\t"<<  _In[nDIM+1] << "\t id:" << _id << "\t"<<_i<<"\n";
		ALLERT(KIRMIZI);func->UYGUNLUK(G_BEST[  0],G_BEST[  0][nDIM],true);
		ALLERT(YESIL);	func->UYGUNLUK(G_BEST[_id],G_BEST[_id][nDIM],true);		
		ALLERT(0);
		}
	}
	return OK2;
...
inline void PSO::ALLERT(const int RENK){
		 if(RENK==YESIL  ){std::cout << "\7" << "\033[1;92m" << std::flush;} // bell yeşil}
	else if(RENK==KIRMIZI){std::cout << "\7" << "\033[1;91m" << std::flush;} // bell kırmızı
