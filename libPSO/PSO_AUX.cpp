...
bool TEKRAR(double **HALK, double *B, const MY::uint &ilk, const MY::uint &son,
		const MY::uint &inPUB, const MY::uint &inDIM) {
	double EPS = 1E-20;
	if (son > inPUB || son <= ilk) {
		return false;
	}
...
/**********************************************************************************************/
bool TEKRAR_FULL(double **HALK, const MY::uint &son, const MY::uint &inDIM) {
	...
	for (MY::uint i1 = 1; i1 < son; i1++) {
		for (MY::uint i2 = i1 - 1; i2 >= 0; i2--) {
			...
			for (MY::uint j = 0; j < inDIM; j++) {
				if (fabs(HALK[i1][j] - HALK[i2][j]) > EPS) {
					VAR = false;
					break;
				}
			}
