...
/*******************************************************************************/
GA::~GA() {
	if (aSNR != NULL) { aSNR = Free_Matrix_1D(aSNR); }
	if (uSNR != NULL) { uSNR = Free_Matrix_1D(uSNR); }
	if (araSNR != NULL) { araSNR = Free_Matrix_1D(araSNR); }
	if (COST != NULL) { COST = Free_Matrix_1D(COST); }
...
GA::GA(KON* _infunc, unsigned int _nI, unsigned int _nB) {
	...
	dist_I.param(std::uniform_int_distribution <unsigned int>::param_type{ 0,nPUB - 1 });
	mt_A.seed(rd_A());
	mt_A.seed(std::chrono::steady_clock::now().time_since_epoch().count());

	init_HAFIZA();
	func->GET_SNR(aSNR, uSNR);
	for (unsigned int j = 0; j < nDIM; j++) {araSNR[j] = uSNR[j] - aSNR[j];}
}
/*******************************************************************************/
void GA::init_POP(bool use_gaussian) {
	...
	*/
	...
	if (use_gaussian) {
		...
		for (unsigned int i = 1; i < nPOP; i++) {
			for (unsigned int j = 0; j < nDIM; j++) {
				dPUB[i][j] = aSNR[j] + araSNR[j] *
				double(int(i / pow(nB, j)) % nB) / double(nB - 1.0);	// dengeli dağıtım
			}
		}
	}
	else {
	...
	double	fmax	 = COST[0];
	double	Tot_Cost = 0;
	for (unsigned int i = 0; i < nPOP; i++) { func->UYGUNLUK(dPUB[i],COST[i]); }
	for (unsigned int i = 0; i < nPOP; i++) {
		fmax = nCOST[i];
		ind = i;
		for (unsigned int j = i + 1; j < nPUB; j++) { if (fmax < nCOST[j]) { fmax = nCOST[j]; ind = j; } }
		for (unsigned int j = 0; j < nDIM; j++) { TAKAS(dPUB[i][j], dPUB[ind][j]); }
		TAKAS(COST[i], COST[ind]);
	}
	for (unsigned int i = 0; i < nPOP; i++) {if (fmax < COST[i]) { fmax = COST[i]; }}
	for (unsigned int i = 0; i < nPOP; i++) {
		nCOST[i] = fmax - COST[i];
		Tot_Cost += nCOST[i];
	}
	for (unsigned int i = 0; i < nPOP; i++) { nCOST[i] /= Tot_Cost; }	// nCOST normalize edildi (0 1] aralığına alındı
	...
	#pragma omp parallel
	{
		...
		for (int i = 1; i < (int)nPOP; i++) {
			...
			for (unsigned int j = 0; j < nDIM; j++) {
				...
				dPUB[S][j] = (cr_d)*dPUB[0][j] + (1 - cr_d) * dPUB[i][j] + (dist_R(mt_A)-0.5)*df;
				if (dist_R(mt_A) < mut_rate) {dPUB[S][j] = aSNR[j] + araSNR[j] * dist_R(mt_A);}
			}
			func->UYGUNLUK(dPUB[S],COST[S]);
			if (COST[S] < COST[i]){for (unsigned int j = 0; j < nDIM; j++) {dPUB[i][j] = dPUB[S][j];}	COST[i]=COST[S];}
		}
	}
	unsigned int	ind = 0; // en büyük değerli bireyin dizideki indisi
	double fmin = COST[0];
	for (unsigned int i = 0; i < nPOP; i++) {if (fmin > COST[i]) { fmin = COST[i]; ind = i; }}
	for (unsigned int j = 0; j < nDIM; j++) { TAKAS(dPUB[0][j], dPUB[ind][j]); }
...
void GA::SEC() {// nPOP 50 cross 0.2- >50+10=60 nElit
	...
	*/
	...
	for (unsigned int i = 0; i < nElit; i++) {
		fmax = nCOST[i];
		ind = i;
		for (unsigned int j = nPOP+i; j < nPUB; j++) { if (fmax < nCOST[j]) { fmax = nCOST[j]; ind = j; } }
		for (unsigned int j = 0; j < nDIM; j++) { TAKAS(dPUB[i][j], dPUB[ind][j]); }
		TAKAS(COST[i], COST[ind]);
//		TAKAS(nCOST[i], nCOST[ind]);
	...
	double	Tot_Cost_2 = 0;
	double sum, r;
	for (unsigned int i = nElit; i < nPUB; i++) { if (fmax < COST[i]) { fmax = COST[i]; }}	// en büyüğün bulunması
	for (unsigned int i = nElit; i < nPUB; i++) { nCOST[i] = fmax - COST[i];	Tot_Cost_1 += nCOST[i];}
	for (unsigned int i = nElit; i < nPUB; i++) { nCOST[i] /= Tot_Cost_1; }	// nCOST normalize edildi (0 1] aralığına alındı

	if (RANK) {
		SIRALA(nPUB, nDIM, nCOST, COST, dPUB);
		for (unsigned int i = 0; i < nPUB; i++) { nCOST[i] = nPUB - i; }	// sıra değeri cost ile değiştirildi
	}
	/* rulet tekerleği yöntemi uygulanacak */
	for (unsigned int i = nElit; i < nPUB; i++) { Tot_Cost_2 += nCOST[i]; }	// ruletin maksimum sayısı belirlendi
	for (unsigned int i = nElit; i < nPOP; i++) {
		r = Tot_Cost_2 * dist_R(mt_A);
		sum = 0;
		unsigned int j = i - 1;
		while (sum < r) { j++; sum += nCOST[j]; }
		Tot_Cost_2 -= nCOST[j];
		TAKAS(COST[i], COST[j]);
		TAKAS(nCOST[i], nCOST[j]);
		for (unsigned int k = 0; k < nDIM; k++) { TAKAS(dPUB[i][k], dPUB[j][k]); }
	}
//	TEKRAR_FULL(dPUB, nPOP,nDIM);
}
/***********************************************************************************/
...
//	static	double old_best = 0;
	...
	*/
	...
	double sd = 0;
//	double best = COST[0];
	for (unsigned int i = 0; i < nPOP; i++) { mean += COST[i]; }
	mean /= nPOP;
	for (unsigned int i = 0; i < nPOP; i++) { sd += (COST[i] - mean) * (COST[i] - mean); }//
	sd = sqrt(sd / nPOP);
	/* hepsi olmak zorunda değil tabii */
	if (fabs(old_mean - mean) < eps &&
		fabs(old_sd - sd) < eps &&
//		old_best == best ||
		sd < 1E-10) {
		return true;
...
double GA::RUN(double* EN_IYI) {
	...
	do{
		...
		if(func->nSON>=func->nDATA){func->nSON=func->nDATA-1;}
		int stage=0;
		do {
	...
	//		//	if (CRITERIA()) { break; std::cout << "CRITERIA "<<iter << std::endl; }
				if(iter%100==0){std::cout<<iter<<"\t"<<COST[0]<<std::endl;}
			}
			use_gaussian=true;
			stage++;
	...
	for (unsigned int i = 0; i < nDIM; i++) { EN_IYI[i] = dPUB[0][i]; }
	return COST[0];
}
/***********************************************************************************/
