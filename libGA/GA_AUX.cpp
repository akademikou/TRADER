...
void TAKAS(double& Var1, double& Var2) { double Temp = Var1;	Var1 = Var2;	Var2 = Temp; }
/**********************************************************************************************/
bool TEKRAR(double** HALK, double* B,
	const unsigned int& ilk,
	const unsigned int& son,
...
//	return false;
	...
	*/
	...
	if (son > nPUB) { return false;}			// programın hatalı davranmaması için her ihtimale karşı
	else if (son <= ilk) { return false; };		// sınır kontrolü
...
bool TEKRAR_FULL(double** HALK,
	...
	for (unsigned int i1 = 1; i1 < son; i1++) {
		for (int i2 = i1-1; i2 >=0; i2--) {
			...
			if (VAR == true) { CNT++; /*return true;*/ }
		}
	}
	if(CNT!=0){std::cout<<CNT<<"\n";}
	return false;
}
/**********************************************************************************************/
void SIRALA(unsigned int num, unsigned int dim, double* L1, double* L2, double** L3) {
	/*
	...
	*/
	...
	for (j = 0; (j <= num) && swapped; j++) {
		...
		for (i1 = 0; i1 < (num - 1); i1++) {
			i2 = i1 + 1;
			if (L1[i2] > L1[i1]) {
				temp = L1[i1];	L1[i1] = L1[i2];	 L1[i2] = temp;
				temp = L2[i1];	L2[i1] = L2[i2];	 L2[i2] = temp;
				for (unsigned int k = 0; k < dim; k++) {
					temp = L3[i1][k];	L3[i1][k] = L3[i2][k]; L3[i2][k] = temp;
				}
				swapped = true;
			}
		}
