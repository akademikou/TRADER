#pragma once
/**********************************************************************************************/
void GET_BEST(std::vector<int> &_BEST,
	...
	while (i < nDATA) {
		...
		for (int j = i; j < nDATA; j++) {
			...
			if ((R_B > 0 || (j == (nDATA - 1) && R_T > 0)
					|| (LAST == (1) && R_T > 0))
					|| (R_S > 0 || (j == (nDATA - 1) && R_T < 0)
							|| (LAST == (-1) && R_T < 0))) {
				if (R_T != 0) {
					LAST = (R_T > 0) ? 1 : (-1);
				}
...
//	for(int i=1;i<nDATA;i++){_BEST[i-1]=_BEST[i];} // LEAD YAPMAK ICIN
	if (TI_Open != NULL) {
		TI_Open = Free_Matrix_1D(TI_Open);
	}
	if (TI_Close != NULL) {
		TI_Close = Free_Matrix_1D(TI_Close);
	}
...
void WORK_SPACE(std::vector<std::vector<double>> &_DATA,
		std::vector<int> &_BEST_poz, std::vector<int> &_BEST_ord) {
...
//	std::vector<int>_BEST_ord;
	...
	for (int i = 1; i < (int) _BEST_poz.size(); i++) {
		if (_BEST_poz[i] != _BEST_poz[i - 1]) {
			_BEST_ord.push_back(_BEST_poz[i]);
		} else {
			_BEST_ord.push_back(0);
		}
	}
...
void LVQ_LEAN(const int &_nGIR, const int &_nCIK, const int &_nPSS,
		const std::vector<int> _BEST_poz, const std::vector<int> _BEST_ord,
...
void PSO_to_R(const int &_nH, const int &_nGIR, const int _nDATA) {
	...
	for (int i = 0; i < (int) PSO_DATA.size(); i++) {
		for (int j = 0; j < (int) PSO_DATA[i].size(); j++) {
