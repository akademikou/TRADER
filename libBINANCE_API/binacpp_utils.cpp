...
int replace_string_once(std::string& str, const char *from, const char *to, int offset) {
    ...
    return int(start_pos) + int(strlen(to));
}
//--------------------------------
...
unsigned long long int get_current_ms_epoch( ) {
    struct timeval tv;
    gettimeofday(&tv, NULL); 
    unsigned long long int VAL = unsigned long long int(tv.tv_sec)* unsigned long long int (1000)+tv.tv_usec / 1000;
    return VAL ;
}
//---------------------------
std::string hmac_sha256( const char *key, const char *data) {
    unsigned char* digest;
    digest = HMAC(EVP_sha256(), key, (int)strlen(key), (unsigned char*)data, strlen(data), NULL, NULL);    
