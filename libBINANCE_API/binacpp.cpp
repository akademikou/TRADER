...
//---------------------------------
void BinaCPP::init(std::string &api_key, std::string &secret_key ) {
...
double BinaCPP::get_price( const char *symbol ){
	...
	std::string str_symbol = string_toupper(symbol);
...
void BinaCPP::get_account( long recvWindow,  Json::Value &json_result ) {
	...
	unsigned long long int SAAT = unsigned long long int(get_current_ms_epoch());
	querystring.append(std::to_string(SAAT) );
...
{	
	...
	post_data.append(std::to_string( amount ));
...
{	
	...
	std::string url(BINANCE_HOST_W);
	url += "/depositHistory.html?";
	std::string action = "GET";
	...
	if ( status > 0 ) {
		...
		querystring.append(std::to_string( status ) );
	}
...
void BinaCPP::get_depositAddress( 
	...
	std::string url(BINANCE_HOST);
	url += "/depositAddress.html?";
	std::string action = "GET";
	
	std::string querystring("asset=");
	querystring.append( asset );
...
//--------------------
// Do the curl
void BinaCPP::curl_api_with_header(std::string &url, std::string &str_result, std::vector <std::string> &extra_http_header , std::string &post_data , std::string &action )
{
